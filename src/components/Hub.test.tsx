...
describe("Hub", () => {
  ...
  it("shows each filter's current value on its own row", () => {
    ...
    const { getByText } = render(<Hub onOpen={() => {}} onOpenMatches={() => {}} />);
  ...
  it("tapping a row opens that row's pane", () => {
    const onOpen = vi.fn();
    const { getByText } = render(<Hub onOpen={onOpen} onOpenMatches={() => {}} />);
  ...
  it("shows the live match count, both totals", async () => {
    const { findByText } = render(<Hub onOpen={() => {}} onOpenMatches={() => {}} />);
  ...
  it("hides the Notifications toggle until the profile actually exists", async () => {
    const { queryByRole, rerender } = render(<Hub onOpen={() => {}} onOpenMatches={() => {}} />);
    expect(queryByRole("switch")).toBeNull();
    act(() => { useWizardStore.getState().set("profile_id", 42); });
    rerender(<Hub onOpen={() => {}} onOpenMatches={() => {}} />);
